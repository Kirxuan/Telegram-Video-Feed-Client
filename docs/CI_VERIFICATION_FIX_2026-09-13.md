...
- Scope：`gradle/verification-metadata.xml`、`.github/workflows/android.yml`、`telegram/src/test/.../TelegramFileManagerTest.kt`、本文。
...
## 等待者取消测试的竞态（2026-09-14 继续验证）

依赖校验修复后的运行 [34757306229](https://github.com/Kirxuan/Telegram-Video-Feed-Client/actions/runs/34757306229) 推进到 Telegram benchmark 单元测试：168 项中仅 `closingALeaseWakesAWaitingLoaderImmediately` 失败。

测试原先的 latch 只表示工作线程进入 Callable，并不保证 `awaitAvailable` 已经经过已关闭租约检查。主线程可能在此间先执行 `close()`，导致工作线程收到 `IllegalStateException`，而测试本来需要验证的是唤醒已阻塞线程时的 `TelegramFileUnavailableException`。使用额外 latch 强制该顺序，已在本机稳定复现同一失败断言。

修复仅调整测试同步：先确认工作线程进入真实的 timed monitor wait，再关闭租约；保留关闭后一秒内完成与取消异常类型的断言。生产取消和唤醒实现未修改。CI 同时保存 Fake 单元测试的 XML 结果，便于后续检查具体失败。

- `:telegram:testBenchmarkUnitTest --tests '*TelegramFileManagerTest' --tests '*LeaseCloseStressTest' --dependency-verification=strict`：通过。临时 stress 测试调用同一测试 100 次，全部通过；临时测试文件和注入脚本保留在忽略的本机 build 报告目录，不纳入源码提交。
- 修复提交后的 GitHub 完整运行：等待对应运行结果，不将历史本机成功视为云端通过。
